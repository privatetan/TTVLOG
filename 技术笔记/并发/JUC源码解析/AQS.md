...
此类旨在为大多数依赖单个原子 int 值来表示状态的同步器提供有用的基础。 
子类必须定义更改此状态的受保护方法，并定义该状态在获取或释放此对象方面的含义。 
鉴于这些，此类中的其他方法执行所有排队和阻塞机制。 
子类可以维护其他状态字段，但仅跟踪使用 getState、setState 和 compareAndSetState 方法操作的原子更新的 int 值以进行同步。

此类支持默认独占模式和共享模式中的一种或两种。 以独占模式获取时，其他线程尝试获取时不会成功。 多个线程的共享模式获取可能（但不一定）成功。
 */
...
public abstract class AbstractQueuedSynchronizer extends AbstractOwnableSynchronizer
    ...
    static final int WAITING   = 1;          // must be 1 ，表示等待队列
    static final int CANCELLED = 0x80000000; // must be negative（负数） ，表示取消等待
    static final int COND      = 2;          // in a condition wait ，表示条件等待
